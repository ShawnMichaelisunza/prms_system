<button onclick="document.getElementById('confirmSupplierDelete{{ $item->id }}').showModal()" type="button"
...
<dialog id="confirmSupplierDelete{{ $item->id }}" class="p-6 bg-white rounded-lg shadow-xl border border-gray-300">
    <form action="{{ route('items.destroy', encrypt($item->id)) }}" method="POST">
        @method('DELETE')
        @csrf
        ...
        <div class="flex justify-between items-center mb-6">
            ...
            <button onclick="document.getElementById('confirmSupplierDelete{{ $item->id }}').close()" type="button"
        ...
        <div class="mb-6 text-gray-700 text-sm">
            Are you sure you want to delete suppplier <span class="font-semibold">{{ $item->item_name }}</span>
            ? </br> This
